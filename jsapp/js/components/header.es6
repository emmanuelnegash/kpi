...
import {assign, currentLang, stringToColor} from 'utils';
...
class MainHeader extends Reflux.Component {
  ...
  toggleLanguageSelector() {
    this.setState({
      isLanguageSelectorVisible: !this.state.isLanguageSelectorVisible,
    });
  }
            ...
            <bem.AccountBox__menu>
              ...
              {shouldDisplayUrls && (
                <bem.AccountBox__menuLI key='2' className='environment-links'>
                  {envStore.data.terms_of_service_url && (
                    <a
                      href={envStore.data.terms_of_service_url}
                      target='_blank'
                    >
                      {t('Terms of Service')}
                    </a>
              ...
              <bem.AccountBox__menuLI m={'lang'} key='3'>
                ...
                {this.state.isLanguageSelectorVisible && (
                  <ul>{langs.map(this.renderLangItem)}</ul>
                )}
              </bem.AccountBox__menuLI>
  ...
  renderGitRevInfo() {
    if (
      ...
      stores.session.currentAccount.git_rev
    ) {
        ...
        <div className='mdl-layout__header-row'>
          ...
          {isLoggedIn && this.isFormList() && (
            <div className='mdl-layout__header-searchers'>
              <ListSearch
                searchContext={this.state.formFiltersContext}
                placeholderText={t('Search Projects')}
              />
            </div>
          )}
