...
import {validFileTypes, isAValidUrl, escapeHtml} from 'utils';
import {NAME_MAX_LENGTH, PROJECT_SETTINGS_CONTEXTS} from 'js/constants';
...
class ProjectSettings extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      'share-metadata': formAsset
        ? formAsset.settings['share-metadata']
        : false,
      // steps
      currentStep: null,
      previousStep: null,
  ...
  componentDidMount() {
    ...
    this.unlisteners.push(
      ...
      actions.resources.updateAsset.failed.listen(
        this.onUpdateAssetFailed.bind(this)
      ),
      actions.resources.cloneAsset.completed.listen(
        this.onCloneAssetCompleted.bind(this)
      ),
      actions.resources.cloneAsset.failed.listen(
        this.onCloneAssetFailed.bind(this)
      ),
      actions.resources.setDeploymentActive.failed.listen(
        this.onSetDeploymentActiveFailed.bind(this)
      ),
      actions.resources.setDeploymentActive.completed.listen(
        this.onSetDeploymentActiveCompleted.bind(this)
      ),
  ...
  onNameChange(evt) {
    ...
    this.onAnyDataChange(
      'name',
      assetUtils.removeInvalidChars(evt.target.value)
    );
  }

  onDescriptionChange(evt) {
    this.setState({
      description: assetUtils.removeInvalidChars(evt.target.value),
    });
    this.onAnyDataChange(
      'description',
      assetUtils.removeInvalidChars(evt.target.value)
  ...
  isArchivable() {
    return (
      this.state.formAsset.has_deployment &&
      this.state.formAsset.deployment__active
    );
  }
  ...
  archiveProject(evt) {
    ...
    this.archiveAsset(
      this.state.formAsset.uid,
      this.onArchiveProjectStarted.bind(this)
    );
  }
  ...
  unarchiveProject(evt) {
    ...
    this.unarchiveAsset(
      this.state.formAsset.uid,
      this.onUnarchiveProjectStarted.bind(this)
    );
  }
    ...
    const assetPromise = new Promise((resolve, reject) => {
      if (this.state.formAsset) {
        ...
        dataInterface
          ...
          .fail(function (r) {
            reject(
              t('Error: asset could not be created.') +
                ` (code: ${r.statusText})`
            );
          });
      }
        ...
        (asset) => {
          ...
          this.applyUrlToAsset(importUrl, asset).then(
            ...
            (response) => {
              ...
              if (importUrl) {
                errLines.push(
                  `<code>Name: ${this.getFilenameFromURI(importUrl)}</code>`
                );
              }
              if (response.messages.error) {
                errLines.push(
                  `<code>${response.messages.error_type}: ${escapeHtml(
  ...
  getNameInputLabel(nameVal) {
    ...
    if (nameVal.length >= NAME_MAX_LENGTH - 99) {
      label += ` (${t('##count## characters left').replace(
        '##count##',
        NAME_MAX_LENGTH - nameVal.length
      )})`;
    }
    return label;
  }
    ...
    return (
      <bem.FormModal__form className='project-settings project-settings--form-source'>
        ...
        <bem.FormModal__item m='form-source-buttons'>
          {this.props.context === PROJECT_SETTINGS_CONTEXTS.NEW && (
            <button
              onClick={this.displayStep.bind(this, this.STEPS.PROJECT_DETAILS)}
            >
      ...
      <bem.FormModal__form className='project-settings project-settings--choose-template'>
        ...
        <bem.Modal__footer>
          ...
          <bem.KoboButton
            ...
            disabled={
              !this.state.chosenTemplateUid || this.state.isApplyTemplatePending
            }
          >
  ...
  renderStepUploadFile() {
    return (
      <bem.FormModal__form className='project-settings'>
        ...
        <bem.Modal__footer>{this.renderBackButton()}</bem.Modal__footer>
      </bem.FormModal__form>
    );
  }
      ...
      <bem.FormModal__form className='project-settings project-settings--import-url'>
        <div className='intro'>
          ...
          {envStore.isReady && envStore.data.support_url && (
            <a
              href={envStore.data.support_url + VIA_URL_SUPPORT_URL}
              target='_blank'
            >
    ...
    return (
      ...
      >
        {this.props.context === PROJECT_SETTINGS_CONTEXTS.EXISTING && (
          <bem.Modal__footer>
            <bem.KoboButton type='submit' m='green' onClick={this.handleSubmit}>
              {t('Save Changes')}
            </bem.KoboButton>
          </bem.Modal__footer>
        )}
          ...
          <bem.FormModal__item m='metadata-share'>
            <Checkbox
              ...
              label={
                ...
                ' ' +
                t(
                  'All the information is submitted anonymously, and will not include the project name or description listed above.'
                )
              }
            />
          </bem.FormModal__item> */}
          ...
          {this.props.context === PROJECT_SETTINGS_CONTEXTS.EXISTING && (
            <bem.FormModal__item>
              <bem.FormModal__item m='inline'>
                {this.isArchived() && (
                  <bem.KoboButton m='blue' onClick={this.unarchiveProject}>
                    {t('Unarchive Project')}
                  </bem.KoboButton>
                )}

                {this.isArchivable() && (
                  <bem.KoboButton m='orange' onClick={this.archiveProject}>
                    {t('Archive Project')}
                  </bem.KoboButton>
                )}
              </bem.FormModal__item>
